 ...
 * sqli_rules.g4）。本头是驱动/调用方与 sqli_rules.rl 之间的唯一
 * 契约：每条 <name>_pat 攻击规则 = 一个独立机器入口
 *   sql_match_sqli_<name>()
 *
 * 匹配语义：输入为 token 类型数组（先经 lex_sql 词法，见
 * sql_tokens.h），从 start 逐位置独立匹配（对齐 rulec/wrapper
 * 逐条独立匹配）；命中写 *len（匹配长度）并返回 1，否则返回 0。
 *
 * 谓词分工：
 ...
 *
 * 用法：include 本头并链接 libragel_sql（含 sqli_rules），调用方
 * 无需接触 .rl 与生成物。
 * ============================================================ */
#ifndef SQLI_RULES_H
#define SQLI_RULES_H
