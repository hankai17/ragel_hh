# 不依赖 cmake 的等价格建：
#   rules/{sql,log4j,xss,js}/*.rl --ragel--> gen/*.c --cc--> libragel_sql.a --link--> 驱动
#   驱动：sql_scan（主目录）+ examples/{sqli,log4j,xss,js}_scan
#   make test 跑五套断言（sql 骨架 / sqli / log4j / xss / js）
...
INC := -I$(SQL_DIR) -I$(LOG4J_DIR) -I$(XSS_DIR) -I$(JS_DIR)
...
all: $(BIN)/sql_scan $(BIN)/sqli_scan $(BIN)/log4j_scan $(BIN)/xss_scan $(BIN)/js_scan
