/* ============================================================
 * sql_syntax.h — RuleSQL 骨架 + 语义谓词库接口
 * ------------------------------------------------------------
 * 对应规则真源 rules/ragel/sql_syntax.rl（对齐归档的 ANTLR 共享骨架
 * RuleSQL.g4）。本头是驱动/调用方与 sql_syntax.rl 之间的唯一契约：
 *   1) CFG 骨架匹配：expr / select_stmt / constant_value
 *      （输入为 token 类型 int 数组，见 sql_tokens.h）
 *   2) 语义谓词：isIdent 与常量规范化相等（谓词复核用）
 *
 * 用法：include 本头并链接 libragel_sql（sql_tokens + sql_syntax +
...
/* ------------------------------------------------------------
 * CFG 骨架匹配（sql_syntax.rl 三个独立机器：expr / select / const）
 * ------------------------------------------------------------
